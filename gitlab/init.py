...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--vault_backend_type",
        default="kv-v2",
        help=" the Vault backend type for the secrets engine",
    )
    ...
    admin_policy = """
    path "kv-v1/cml/holops2800" {
        capabilities = ["create", "read", "update", "delete", "list"]
    }
    """
    created_policy = client.sys.create_or_update_policy("cml-access", admin_policy)
    logger.debug("created/updated policy response=%s", created_policy)

    created_role = client.create_role("holops2800", policies="cml-access")
    logger.debug("created role response=%s", created_role)

    role_id = client.get_role_id("holops2800")
    secret_id = client.create_role_secret_id("holops2800")
    ...
    logger.info("vault_mount_path=%s", "kv-v1/cml/holops2800")
